...
create policy if not exists "Users can select own mappings"
  on public.noco_space_owners
  for select
  to authenticated
  using (auth.uid() = user_id);

create policy if not exists "Users can insert own mappings"
  on public.noco_space_owners
  for insert
  to authenticated
  with check (auth.uid() = user_id);

create policy if not exists "Users can delete own mappings"
  on public.noco_space_owners
  for delete
  to authenticated
  using (auth.uid() = user_id);
